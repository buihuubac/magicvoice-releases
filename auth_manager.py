...
def _get_client_version() -> str:
    """Doc version.txt cung thu muc - gui kem /verify de server co the
    tu choi client qua cu (vd sau khi vá lo hong bao mat, bat khach
    cap nhat truoc khi dang nhap tiep). Rong neu khong doc duoc, server
    se tu coi la ban rat cu."""
    try:
    ...
    for _attempt in range(3):
        try:
            ...
            if data.get("ok"):
                ...
                # FIX (bao mat 2026-08-14, theo yeu cau anh Bac): server co
                # the tra ve loi "phien ban qua cu" khi client duoi
                # MIN_CLIENT_VERSION - KHONG duoc chua bat ky tu khoa nao
                # trong _is_net_err (ket noi/timeout/connection/server) o
                # login(), neu khong se bi hieu nham la loi mang va roi vao
                # fallback verify_login_offline() (khong kiem tra version,
                # vo hieu hoa hoan toan muc dich chan nay).
