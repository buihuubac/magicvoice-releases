...
def check_for_update(root, silent=False):
    ...
    def _do_update(new_ver):
        """v3.36: Tai bo .pyd moi -> goi updater.bat -> tu khoi dong lai.

        Khac biet voi v3.35 (tai .py + de truc tiep):
        - Windows khoa file .pyd dang chay -> khong de duoc
        - Phai tai thanh .pyd.new -> dong app -> updater.bat swap
        """
        try:
            import urllib.request, shutil, subprocess as _sp, sys as _sys

            # Folder cua app (chua magicvoice.py + cac .pyd)
            app_dir = Path(__file__).resolve().parent

            # Danh sach file can tai (tat ca tu repo magicvoice-releases)
            BASE_URL = "https://raw.githubusercontent.com/buihuubac/magicvoice-releases/main"
            FILES_TO_UPDATE = [
                ("magicvoice_core.cp311-win_amd64.pyd",  f"{BASE_URL}/magicvoice_core.cp311-win_amd64.pyd"),
                ("auth_manager.cp311-win_amd64.pyd",     f"{BASE_URL}/auth_manager.cp311-win_amd64.pyd"),
                ("license_guard.cp311-win_amd64.pyd",    f"{BASE_URL}/license_guard.cp311-win_amd64.pyd"),
                ("magicvoice.py",                         f"{BASE_URL}/magicvoice.py"),
                ("script_processor.py",                   f"{BASE_URL}/script_processor.py"),
                ("setup_helper.py",                       f"{BASE_URL}/setup_helper.py"),
                ("CaiDat_MagicVoice.bat",                 f"{BASE_URL}/CaiDat_MagicVoice.bat"),
                ("updater.bat",                           f"{BASE_URL}/updater.bat"),
                ("version.txt",                           f"{BASE_URL}/version.txt"),
            ]
            ...
            # Tai tung file vao .new (de tranh dung file dang chay)
            total = len(FILES_TO_UPDATE)
            new_files = []  # list (final_path, new_path) de updater.bat swap
            try:
                for idx, (fname, furl) in enumerate(FILES_TO_UPDATE, 1):
                    status_lbl.config(text=f"[{idx}/{total}] {fname}")
                    prog.update()

                    final_path = app_dir / fname
                    # File .pyd -> tai thanh .new (updater.bat se swap)
                    # File .py / .bat / .txt -> tai thanh .new luon cho an toan
                    new_path = app_dir / (fname + ".new")
                    urllib.request.urlretrieve(furl, str(new_path))

                    # Verify file tai ve khong rong
                    if new_path.stat().st_size == 0:
                        raise RuntimeError(f"File {fname} tai ve rong (0 bytes)")

                    new_files.append((final_path, new_path))

                    # Update progress bar
                    bar.config(width=int(360 * idx / total))
                    prog.update()

            except Exception as _dl_err:
                # Rollback: xoa tat ca file .new da tai
                for _fp, _np in new_files:
                    try: _np.unlink()
                    except: pass
                # Xoa file .new chua kip them vao list
                for fname, _ in FILES_TO_UPDATE:
                    try: (app_dir / (fname + ".new")).unlink()
                    except: pass
                raise RuntimeError(f"Tai file that bai: {_dl_err}") from _dl_err

            # Xoa cache cu (giong logic v3.35)
            _cache_cleared = []
            try:
                # 1. Xoa __pycache__
                _pycache = app_dir / "__pycache__"
                if _pycache.exists():
                    shutil.rmtree(str(_pycache), ignore_errors=True)
                    _cache_cleared.append("__pycache__")

                # 2. Xoa .deps_installed flag
                _deps_flag = app_dir / ".deps_installed"
                if _deps_flag.exists():
                    _deps_flag.unlink()
                    _cache_cleared.append(".deps_installed")

                # 3. Xoa _trim*.wav cache
                for _trim_file in app_dir.rglob("*_trim*.wav"):
                    try:
                        _trim_file.unlink()
                        _cache_cleared.append(_trim_file.name)
                    except: pass

                # 4. Reset ref_text rac trong voices_library.json
                _voices_file = app_dir / "voices_library.json"
            ...
            status_lbl.config(text="Hoan tat, dang khoi dong lai...")
            prog.update()
            import time
            time.sleep(0.5)
            prog.destroy()
            ...
            messagebox.showinfo(
                "Cap nhat thanh cong!",
                f"Da tai xong v{new_ver}!{_cache_msg}\n\n"
                "App se tu khoi dong lai ngay bay gio.")

            # Goi updater.bat (file MOI vua tai ve dang la updater.bat.new)
            # Ta phai swap updater.bat truoc khi chay
            new_updater = app_dir / "updater.bat.new"
            final_updater = app_dir / "updater.bat"
            if new_updater.exists():
                try:
                    if final_updater.exists():
                        final_updater.unlink()
                    new_updater.rename(final_updater)
                except Exception:
                    # Neu khong swap duoc (file lock), van dung updater.bat cu
                    pass

            # Chay updater.bat -> tu kill app + swap cac file .pyd.new + mo lai app
            # Truyen PID hien tai de updater biet wait cho process nao
            import os as _os
            _pid = _os.getpid()

            # FIX: DETACHED_PROCESS (0x08) voi cmd.exe gay ra cua so CMD moi (bug).
            # Dung STARTUPINFO.wShowWindow=SW_HIDE + CREATE_NEW_PROCESS_GROUP:
            #   - SW_HIDE: an cua so CMD ngay khi khoi tao (khong hien ra man hinh)
            #   - CREATE_NEW_PROCESS_GROUP (0x200): process tiep tuc sau khi app dong
            #   - Bo "start /B": chay batch truc tiep qua cmd /C, don gian hon
            _si = _sp.STARTUPINFO()
            _si.dwFlags = _sp.STARTF_USESHOWWINDOW
            _si.wShowWindow = 0   # SW_HIDE
            _sp.Popen(
                ["cmd.exe", "/C", str(final_updater), str(_pid)],
                cwd=str(app_dir),
                creationflags=0x00000200,  # CREATE_NEW_PROCESS_GROUP
                startupinfo=_si,
            )

            # Dong app de updater.bat co the swap file
            root.after(500, lambda: (root.destroy(), _sys.exit(0)))
...
class App(tk.Tk):
    def __init__(self, login_msg="", username=""):
        ...
        self.gap_var      =tk.IntVar(value=150)
    ...
    def _build_script_tab(self, p):
        ...
        tk.Label(sbar, text="Gap:",
                 font=(FN,8), bg=P["sidebar"], fg=P["dim"]).pack(side="left")
        # v3.37: Dung CHUNG self.gap_var voi tab Phu De SRT (sync 2 chieu)
        # KHONG khai bao lai self.srt_gap nua.
                            ...
                            if not os.path.exists(mp3_path):
                                ...
                                if sz < 100:
                                    ...
                                    try:
                                        ...
                                        _wv, _sr = torchaudio.load(_wav_tmp11)
                                    finally:
                                    ...
                                    torchaudio.save(out_path, _wv, 24000)
                                    try: os.remove(mp3_path)
                                    except Exception: pass
                                    return True
                    except Exception as e:
    ...
    def _run_srt_edge(self, entries, vp):
                            ...
                            if sz >= 100:
                                ...
                                try:
                                    _sp11b.run(
                                        ...
                                        timeout=30, check=True,
                                        creationflags=0x08000000)
                                    _wv, _sr = _ta_one.load(_wav_tmp11b)
                                finally:
                                ...
                                if _wv.shape[0] > 1:
                                    _wv = _wv.mean(dim=0, keepdim=True)
                                _ta_one.save(tmp_wav, _wv, 24000)
        ...
        for i, e in enumerate(entries):
            ...
            try:
                ...
                tmp_wav = asyncio.run(_gen_one(txt_for_tts, voice_id))
                ...
                # v3.37: them silence CUOI moi file le = gap_var
                # de khach keo vao CapCut co nhip nghi tu nhien
                _t_with_gap = torch.cat([t, silence], dim=-1)
                to_mp3(_t_with_gap, _part_mp3)
            ...
            for i, e in enumerate(entries):
                ...
                try:
                    ...
                    if len(txt) <= _MAX_CH:
                        ...
                        _sents = _re12.split(r'(?<=[.!?])\s+', txt.strip())
                        _chunks = []; _buf = ""
                        for _s in _sents:
                            _s = _s.strip()
                            if not _s: continue
                            if not _buf: _buf = _s
                            elif len(_buf) + 1 + len(_s) <= _MAX_CH: _buf += ' ' + _s
                            else: _chunks.append(_buf); _buf = _s
                        if _buf: _chunks.append(_buf)
                        if not _chunks: _chunks = [txt]
                    # Gen tung chunk + 350ms silence GIUA (sau dau cham, khong them cuoi)
                    _join_sil = torch.zeros(1, int(0.35 * SR))  # 350ms @ 24kHz
                    _parts = []
                    for _ci, _ck in enumerate(_chunks):
                        _a = Backend.gen(_ck, num_step=self.steps_var.get(),
                                         speed=self._get_speed(), **kw)
                        _parts.append(_to_tensor(_a))
                        if _ci < len(_chunks) - 1:
                            _parts.append(_join_sil)
                    audio_t = torch.cat(_parts, dim=-1)
                    # v3.37: them silence CUOI moi file le = gap_var (vd 150ms)
                    # de khach keo vao CapCut co nhip nghi tu nhien
                    _le_silence = torch.zeros(1, int(self.gap_var.get() * SR / 1000))
                    all_parts.append(torch.cat([audio_t, _le_silence], dim=-1))
